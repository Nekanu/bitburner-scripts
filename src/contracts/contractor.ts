...
export async function main(ns) {
    ...
    let dictContractDataStrings = await contractsDictCommand('JSON.stringify(ns.codingcontract.getData(c.contract, c.hostname), jsonReplacer)', '/tmp/contract-data-stringified.txt');
    ...
    ns.run(getFilePath('/contracts/run-with-delay.js'), { temporary: true }, scriptSolver, 1, JSON.stringify(contractsDb));
}
